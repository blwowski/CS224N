...
class NMT(nn.Module):
    ...
    def __init__(self, embed_size, hidden_size, vocab, dropout_rate=0.2):
        ...
        ###     self.decoder (LSTM Cell with bias)  input into decoder = h_enc, c_enc
        self.decoder = nn.LSTMCell(hidden_size + embed_size, hidden_size, bias=True)

        ###     self.h_projection (Linear Layer with no bias), called W_{h} in the PDF.
        ###     input size = hidden_size_forward + hidden_size_backwards
        self.h_projection = nn.Linear(hidden_size * 2, hidden_size, bias=False)

        ###     self.c_projection (Linear Layer with no bias), called W_{c} in the PDF.
        ###     input size = cellstate_forward + cellstate_backwards (encoder output size = hidden_size)
    ...
    def step(self, Ybar_t: torch.Tensor,
        ...
        ###     unsqueeze(2) will turn dec_hidden to 3 dimensions (b, h, 1)
        ###     squeeze(2) will force the result of enc_hiddens * dec_hidden to 2 dimensions
        ###     Equation 7 PDF enc_hiddens_proj == W_attProj * h_enc
        ...
        ###     dim=0 would be batch. dim=1 is src_len
        ###     Equation 8
